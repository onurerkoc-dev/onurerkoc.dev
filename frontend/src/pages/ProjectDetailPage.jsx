import { useEffect, useState } from 'react'
import { Link, useParams } from 'react-router'
import {
  getProjectBySlug,
  getProjects,
} from '../api/projectsApi'
import Seo from '../components/Seo'
import SiteFooter from '../components/SiteFooter'

function formatUpdatedAt(value) {
  if (!value) {
    return 'Active development'
  }

  const parsedDate = new Date(value)

  if (Number.isNaN(parsedDate.getTime())) {
    return value
  }

  return new Intl.DateTimeFormat('en', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  }).format(parsedDate)
}

function getIsoDate(value) {
  if (!value) {
    return null
  }

  const parsedDate = new Date(value)

  if (Number.isNaN(parsedDate.getTime())) {
    return null
  }

  return parsedDate.toISOString()
}

const sectionLinks = [
  ['overview', 'Overview'],
  ['problem', 'Problem'],
  ['goal', 'Goal'],
  ['architecture', 'Architecture'],
  ['decisions', 'Decisions'],
  ['roadmap', 'Roadmap'],
]

function ProjectDetailPage() {
  const { slug } = useParams()
  const [project, setProject] = useState(null)
  const [allProjects, setAllProjects] = useState([])
  const [status, setStatus] = useState('loading')
  const [errorMessage, setErrorMessage] = useState('')

  const canonicalPath = `/projects/${slug}`
  const canonicalUrl =
    `https://onurerkoc.dev${canonicalPath}`

  const projectTechStack = project?.techStack ?? []
  const keyDecisions = project?.keyDecisions ?? []
  const nextSteps = project?.nextSteps ?? []

  const normalizedUpdatedAt =
    status === 'success' && project
      ? getIsoDate(project.updatedAt)
      : null

  const seoTitle =
    status === 'success' && project
      ? `${project.title} | Onur Erkoç`
      : status === 'error'
        ? 'Project Not Found | Onur Erkoç'
        : 'Project Case Study | Onur Erkoç'

  const seoDescription =
    status === 'success' && project
      ? project.summary
      : status === 'error'
        ? 'The requested project could not be found on onurerkoc.dev.'
        : 'Engineering project case study by Onur Erkoç.'

  const projectStructuredData =
    status === 'success' && project
      ? {
          '@context': 'https://schema.org',
          '@type': 'SoftwareSourceCode',
          name: project.title,
          description: project.summary,
          url: canonicalUrl,
          keywords: projectTechStack.join(', '),
          author: {
            '@type': 'Person',
            name: 'Onur Erkoç',
            url: 'https://onurerkoc.dev/',
          },
          ...(normalizedUpdatedAt
            ? { dateModified: normalizedUpdatedAt }
            : {}),
          ...(project.githubUrl
            ? { codeRepository: project.githubUrl }
            : {}),
        }
      : null

  const sortedProjects = [...allProjects].sort(
    (firstProject, secondProject) =>
      Number(secondProject.featured) -
      Number(firstProject.featured)
  )

  const currentProjectIndex = sortedProjects.findIndex(
    (item) => item.slug === slug
  )

  const previousProject =
    currentProjectIndex > 0
      ? sortedProjects[currentProjectIndex - 1]
      : null

  const nextProject =
    currentProjectIndex >= 0 &&
    currentProjectIndex < sortedProjects.length - 1
      ? sortedProjects[currentProjectIndex + 1]
      : null

  useEffect(() => {
    window.scrollTo({
      top: 0,
      behavior: 'auto',
    })
  }, [slug])

  useEffect(() => {
    const controller = new AbortController()

    setProject(null)
    setStatus('loading')
    setErrorMessage('')

    async function loadProject() {
      try {
        const data = await getProjectBySlug(slug, {
          signal: controller.signal,
        })

        setProject(data)
        setStatus('success')
      } catch (error) {
        if (error.name === 'AbortError') {
          return
        }

        setErrorMessage('Project could not be found.')
        setStatus('error')
      }
    }

    async function loadProjectNavigation() {
      try {
        const data = await getProjects({
          signal: controller.signal,
        })

        setAllProjects(data)
      } catch (error) {
        if (error.name !== 'AbortError') {
          setAllProjects([])
        }
      }
    }

    loadProject()
    loadProjectNavigation()

    return () => {
      controller.abort()
    }
  }, [slug])

  return (
    <main id="top" className="wbSite wbProjectPage">
      <Seo
        title={seoTitle}
        description={seoDescription}
        path={canonicalPath}
        robots={
          status === 'error'
            ? 'noindex, nofollow'
            : 'index, follow'
        }
        type={
          status === 'success'
            ? 'article'
            : 'website'
        }
        structuredData={projectStructuredData}
      />

      <nav className="wbProjectTopbar">
        <Link to="/#work">← all projects</Link>
        <span>/projects/{slug}</span>
        <a
          href="https://github.com/onurerkoc-dev"
          target="_blank"
          rel="noreferrer"
        >
          github ↗
        </a>
      </nav>

      {status === 'loading' && (
        <section className="wbProjectState">
          <span />
          GET /api/projects/{slug}
        </section>
      )}

      {status === 'error' && (
        <section className="wbProjectState wbProjectStateError">
          <strong>404 / case study unavailable</strong>
          <p>{errorMessage}</p>
          <Link to="/#work">return to repository →</Link>
        </section>
      )}

      {status === 'success' && project && (
        <article className="wbCaseStudy">
          <header className="wbCaseHero">
            <div className="wbCaseOverline">
              <span>{project.type}</span>
              <span>{project.status}</span>
            </div>

            <h1>{project.title}</h1>

            <p>{project.summary}</p>

            <div className="wbCaseMeta">
              <div>
                <span>updated</span>
                <strong>
                  {formatUpdatedAt(project.updatedAt)}
                </strong>
              </div>

              <div>
                <span>stack size</span>
                <strong>{projectTechStack.length}</strong>
              </div>

              <div>
                <span>route</span>
                <strong>/projects/{slug}</strong>
              </div>
            </div>

            <div className="wbCaseStack">
              {projectTechStack.map((technology) => (
                <span key={technology}>{technology}</span>
              ))}
            </div>

            {(project.githubUrl || project.liveUrl) && (
              <div className="wbCaseActions">
                {project.liveUrl && (
                  <a
                    href={project.liveUrl}
                    target="_blank"
                    rel="noreferrer"
                  >
                    open live system ↗
                  </a>
                )}

                {project.githubUrl && (
                  <a
                    href={project.githubUrl}
                    target="_blank"
                    rel="noreferrer"
                  >
                    inspect source ↗
                  </a>
                )}
              </div>
            )}
          </header>

          <div className="wbCaseLayout">
            <aside className="wbCaseNav">
              <span>case index</span>

              <nav aria-label="Case study sections">
                {sectionLinks.map(([id, label], index) => (
                  <a key={id} href={`#${id}`}>
                    <span>
                      {String(index + 1).padStart(2, '0')}
                    </span>
                    {label}
                  </a>
                ))}
              </nav>
            </aside>

            <div className="wbCaseContent">
              <section id="overview" className="wbCaseSection">
                <div className="wbCaseSectionTitle">
                  <span>01</span>
                  <div>
                    <small>implementation overview</small>
                    <h2>What was built</h2>
                  </div>
                </div>

                <p className="wbCaseLead">
                  {project.description}
                </p>
              </section>

              <div className="wbCaseSplit">
                <section id="problem" className="wbCaseSection">
                  <div className="wbCaseSectionTitle">
                    <span>02</span>
                    <div>
                      <small>context</small>
                      <h2>Problem</h2>
                    </div>
                  </div>

                  <p>{project.problem}</p>
                </section>

                <section id="goal" className="wbCaseSection">
                  <div className="wbCaseSectionTitle">
                    <span>03</span>
                    <div>
                      <small>direction</small>
                      <h2>Goal</h2>
                    </div>
                  </div>

                  <p>{project.goal}</p>
                </section>
              </div>

              <section
                id="architecture"
                className="wbCaseSection wbArchitecture"
              >
                <div className="wbCaseSectionTitle">
                  <span>04</span>
                  <div>
                    <small>system design</small>
                    <h2>Architecture</h2>
                  </div>
                </div>

                <div className="wbArchitectureNote">
                  <span>architecture note</span>
                  <p>{project.architecture}</p>
                </div>

                <div className="wbArchitectureFlow">
                  <span>interface</span>
                  <strong>→</strong>
                  <span>api</span>
                  <strong>→</strong>
                  <span>service</span>
                  <strong>→</strong>
                  <span>database</span>
                </div>
              </section>

              <div className="wbCaseSplit">
                <section id="decisions" className="wbCaseSection">
                  <div className="wbCaseSectionTitle">
                    <span>05</span>
                    <div>
                      <small>engineering</small>
                      <h2>Key decisions</h2>
                    </div>
                  </div>

                  <ol className="wbCaseList">
                    {keyDecisions.map((decision, index) => (
                      <li key={decision}>
                        <span>
                          {String(index + 1).padStart(2, '0')}
                        </span>
                        <p>{decision}</p>
                      </li>
                    ))}
                  </ol>
                </section>

                <section id="roadmap" className="wbCaseSection">
                  <div className="wbCaseSectionTitle">
                    <span>06</span>
                    <div>
                      <small>development roadmap</small>
                      <h2>Next steps</h2>
                    </div>
                  </div>

                  <ol className="wbCaseList">
                    {nextSteps.map((step, index) => (
                      <li key={step}>
                        <span>
                          {String(index + 1).padStart(2, '0')}
                        </span>
                        <p>{step}</p>
                      </li>
                    ))}
                  </ol>
                </section>
              </div>
            </div>
          </div>

          {(previousProject || nextProject) && (
            <nav
              className="wbCasePagination"
              aria-label="Project navigation"
            >
              {previousProject ? (
                <Link
                  to={`/projects/${previousProject.slug}`}
                >
                  <span>← previous</span>
                  <strong>{previousProject.title}</strong>
                </Link>
              ) : (
                <span />
              )}

              {nextProject && (
                <Link
                  to={`/projects/${nextProject.slug}`}
                  className="wbCasePaginationNext"
                >
                  <span>next →</span>
                  <strong>{nextProject.title}</strong>
                </Link>
              )}
            </nav>
          )}
        </article>
      )}

      <SiteFooter />
    </main>
  )
}

export default ProjectDetailPage
